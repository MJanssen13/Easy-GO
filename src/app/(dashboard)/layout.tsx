...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ModuleTheme className="app-surface flex min-h-screen flex-col">
      <header className="sticky top-0 z-20 border-b print:hidden border-white/10 bg-[linear-gradient(105deg,hsl(var(--grad-from)),hsl(var(--grad-via))_52%,hsl(var(--grad-to)))] text-white shadow-soft">
      ...
      <main className="w-full flex-1 px-4 py-6 sm:px-6 lg:px-8 print:p-0">{children}</main>

      <footer className="border-t bg-white/70 print:hidden px-4 py-3 text-center text-xs text-muted-foreground">
